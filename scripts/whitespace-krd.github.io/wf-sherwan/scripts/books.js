...
function startAnimation(yearDiv) {
  ...
  bookElements.forEach((book, index) => {
    ...
    const bookTween = gsap.timeline()
      .to(book, { x: '0%', duration: 0, ease: "expo" })
      .to(book, { x: '-50%', rotation: -20, duration: 0.5, ease: "expo" }, index * 5)
      .call(() => {
...
yearButtons.forEach(button => {
  ...
  button.addEventListener('click', () => {
    ...
    yearDivs.forEach(div => {
      gsap.to(div, { translateY: '-5%', opacity: 0, duration: 0.4, ease: "bouncy" });
    });

    // Show the selected year div
    const selectedYearDiv = document.querySelector(`.year-${selectedYear}`);
    gsap.to(selectedYearDiv, { translateY: '0%', opacity: 1, duration: 0.4, ease: "bouncy" });
